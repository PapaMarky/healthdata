...
class GraphLayer:
    ...
    def redraw(self, base_surface: pygame.Surface):
        ...
        :param base_surface: The surface to draw on
        :return:
        """
        if self.visible:
            if self.dirty:
...
class BackgroundLayer(GraphLayer):
    def __init__(self, size, bg_color: pygame.Color, x_scaler: DataViewScaler, y_scaler: DataViewScaler, offset=(0, 0)):
...
class BPRangeLayer(BackgroundLayer):
    def __init__(self, size, color: pygame.Color, x_scaler: DataViewScaler, y_scaler: DataViewScaler):
        super(BPRangeLayer, self).__init__(size, color, x_scaler, y_scaler)

    def update(self):
        self.update_surface()
        if not self.dirty or not self.visible:
            return
        # 120 / 80 : "normal" range for BP
        y0 = self._y_scaler.scale(120)
        y1 = self._y_scaler.scale(80)
        x0 = self._x_scaler.view_min
        x1 = self._x_scaler.view_max
...
class AxisLayer(BackgroundLayer):
    def __init__(self, size, color: pygame.Color, width: int,
                 x_scaler: DataViewScaler, y_scaler: DataViewScaler, graph_rect):
        super(AxisLayer, self).__init__(size, pygame.Color(0, 0, 0, 0), x_scaler=x_scaler, y_scaler=y_scaler)
        self.x_axis_rect: Union[pygame.Rect, None] = None
        self.y_axis_rect: Union[pygame.Rect, None] = None
...
class DataSetLayer(BackgroundLayer):
    def __init__(self, data_set: DataSet, size, x_scaler: DataViewScaler, y_scaler: DataViewScaler):
        super(DataSetLayer, self).__init__(size, pygame.Color(0, 0, 0, 0), x_scaler, y_scaler)
...
class DataSeriesLayer(DataSetLayer):
    def __init__(self, data_set: DataSeries, size, x_scaler: DataViewScaler, y_scaler: DataViewScaler):
        if not isinstance(data_set, DataSeries):
            raise Exception('DataSeriesLayer data_set must be DataSeries instance')
        super(DataSeriesLayer, self).__init__(data_set, size, x_scaler, y_scaler)
        self.data_set: DataSeries = data_set

    def update(self):
        super().update()
        lines = []
        y_count = self.data_set.y_per_column
        for y in range(y_count):
            print(f'Start Empty Line: {y}')
            lines.append([])

        if self.data_set.data_count > 0:
            print(f'First Row: {self.data_set.get_row(0)["startDate"]}')
            print(f' Last Row: {self.data_set.get_row(self.data_set.data_count - 1)["startDate"]}')
        else:
            print(f'NO DATA')
        for row in self.data_set:
            x = self._x_scaler.scale(float(row[0]))
            for i in range(y_count):
                y = self._y_scaler.scale(float(row[i + 1]))
                lines[i].append([x, y])
...
class DateRangeDataLayer(DataSetLayer):
    def __init__(self, data_set, size,
                 x_scaler: DataViewScaler, y_scaler: DataViewScaler,
                 type_color_map: dict, type_row: int):
        ...
        super(DateRangeDataLayer, self).__init__(data_set, size, x_scaler, y_scaler)
        self.data_set: DataDateRange = data_set
    ...
    def update(self):
        ...
        top = self._y_scaler.view_max
        bottom = self._y_scaler.view_min
        h = bottom - top
        y_offset = 0
        for row in self.data_set:
            ...
            left = self._x_scaler.scale(row[1])
            right = self._x_scaler.scale(row[2])
            w = right - left
            r = pygame.Rect(left, bottom - y_offset - h, w, h)
...
class UIGraph(UIImage):
    ...
    def __init__(self,
        ...
        self.graph_data_rect: Union[pygame.Rect, None] = None
        self.graph_rect: Union[pygame.Rect, None] = None
        self.title_rect: Union[pygame.Rect, None] = None
        self.data_sets = []
        self._layer_list = []
        self.sleep_data = None
        self._x_scaler = DataViewScaler([None, None], [None, None])
        self._y_scaler = DataViewScaler([0, 200], [None, None])
        ...
        self.title_layer = TextLayer(title,
                                     font_name=title_font_name,
                                     font_size=title_font_size,
                                     color=title_text_color)
        # need to create text surface before recalculate_layout
        self.title_layer.update()
        self.add_layer(self.title_layer)

        self.calculate_graph_rect()
        origin = self.graph_rect.topleft
        self.background_layer = BackgroundLayer(self.graph_rect.size, pygame.Color(220, 220, 220, 255),
                                                self._x_scaler, self._y_scaler, offset=origin)
        self.add_layer(self.background_layer)

        self.axis_layer = AxisLayer(self.get_relative_rect().size, self.axis_color, self.axis_width,
                                    self._x_scaler, self._y_scaler, self.graph_rect)
        ...
        data_bg_layer = BackgroundLayer((w, h), pygame.Color(255, 255, 255, 255),
                                        self._x_scaler, self._y_scaler, self.axis_layer.y_axis_rect.topright)
        self.add_layer(data_bg_layer)

        self.add_layer(self.axis_layer)

        bp_range_layer = BPRangeLayer(self.get_relative_rect().size, pygame.Color(220, 255, 220, 128),
                                      self._x_scaler, self._y_scaler)
    ...
    def save_to_image(self, output_path):
        pygame.image.save(self.base_surface, output_path)
    ...
    def add_data_series_layer(self, data_series: DataSeries):
        minimum_x, maximum_x = data_series.x_minmax
        self._x_scaler.update_data_limits(minimum_x, maximum_x)
        minimum_y, maximum_y = data_series.y_minmax
        self._y_scaler.update_data_limits(minimum_y, maximum_y)
        layer = DataSeriesLayer(data_series, self.get_relative_rect().size, self._x_scaler, self._y_scaler)
        self._layer_list.append(layer)
    ...
    def add_sleep_data(self, sleep_data: DataDateRange):
        ...
        self._x_scaler.update_data_limits(sleep_data.minimum_x, sleep_data.maximum_x)
        type_row = 0
        type_color_map = {
        ...
        layer = DateRangeDataLayer(sleep_data, self.get_relative_rect().size,
                                   self._x_scaler, self._y_scaler, type_color_map, type_row)
    ...
    def recalculate_layout(self):
        ...
        self._x_scaler.view_min = self.graph_data_rect.left
        self._x_scaler.view_max = self.graph_data_rect.right
        self._y_scaler.view_min = self.graph_data_rect.bottom
        self._y_scaler.view_max = self.graph_data_rect.top
